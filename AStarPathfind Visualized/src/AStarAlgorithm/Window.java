...
public class Window extends Application
{
    ...
    public static double w = scrW / (double) cols;
    public static double h = scrH / (double) rows;
    ...
    public void start(Stage stage) throws Exception
    {
        ...
        back.setFill(Color.rgb(200,200,200));
        //
        ...
        loop = new Timeline(new KeyFrame(Duration.millis(16), e -> {
            ...
            for (int i = 0; i < cols; i++)
            {
                for (int j = 0; j < rows; j++)
                {
                    grid[i][j].show(Color.rgb(200,200,200));
                }
            }
            start.show(Color.rgb(0,255,0));

            end.show(Color.rgb(255,0,0));
/*
...
*/
            ...
            root.getChildren().add(optPath.updatePath(path));
        ...
        */
        ...
        for (int i = 0; i < cols; i++)
        {
            for (int j = 0; j < rows; j++)
            {
                root.getChildren().add(grid[i][j].getBody());
            }
        }
        root.getChildren().add(optPath.getBody());
